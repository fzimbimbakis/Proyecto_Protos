...
//                        printf("FD: %d, TIME %d\n", item->fd, item->time);
            ...
            if (difftime(time(NULL), item->lastConnection) >= 15) {
//                printf("entro el diff time\n");
                if (item->handler->handle_close != NULL){
                    printf("entro el null %d\n", item->fd);
                    int aux = item->fd;
                    if(SELECTOR_SUCCESS != selector_unregister_fd(key.s, item->fd)) {
                        abort();
                    }
                    close(aux);
//                    selector_unregister_fd(s, item->fd);
                }
            }
        }
    }
